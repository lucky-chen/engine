...
public class FlutterJNI {
    ...
    public static native void nativeInit(@NonNull Context context, @NonNull String[] args,
            @Nullable String bundlePath, @NonNull String appStoragePath,
    ...
    /**
     * Returns true if this instance of {@code FlutterJNI} is connected to Flutter's native engine
     * via a Java Native Interface (JNI).
     */
     ...
     *
     * <p>Invoking this method will result in the release of all native-side resources that were
     ...
     * paint.
     *
     * <p>See {@link android.view.SurfaceHolder.Callback#surfaceCreated(SurfaceHolder)} for an
     ...
     *
     * <p>See {@link android.view.SurfaceHolder.Callback#surfaceChanged(SurfaceHolder, int, int,
     * int)} for an example of where this call might originate.
     */
    @UiThread
     ...
     *
     * <p>This method should be invoked with initial values upon attaching to native. Then, it
     * should be invoked any time those metrics change while {@code FlutterJNI} is attached to
     * native.
     */
    @UiThread
    public void setViewportMetrics(float devicePixelRatio, int physicalWidth, int physicalHeight,
            int physicalPaddingTop, int physicalPaddingRight, int physicalPaddingBottom,
            int physicalPaddingLeft, int physicalViewInsetTop, int physicalViewInsetRight,
            int physicalViewInsetBottom, int physicalViewInsetLeft, int systemGestureInsetTop,
            int systemGestureInsetRight, int systemGestureInsetBottom, int systemGestureInsetLeft) {
        ensureRunningOnMainThread();
        ensureAttachedToNative();
        nativeSetViewportMetrics(nativePlatformViewId, devicePixelRatio, physicalWidth,
                physicalHeight, physicalPaddingTop, physicalPaddingRight, physicalPaddingBottom,
                physicalPaddingLeft, physicalViewInsetTop, physicalViewInsetRight,
                physicalViewInsetBottom, physicalViewInsetLeft, systemGestureInsetTop,
                systemGestureInsetRight, systemGestureInsetBottom, systemGestureInsetLeft);
    }

    private native void nativeSetViewportMetrics(long nativePlatformViewId, float devicePixelRatio,
            int physicalWidth, int physicalHeight, int physicalPaddingTop, int physicalPaddingRight,
            int physicalPaddingBottom, int physicalPaddingLeft, int physicalViewInsetTop,
            int physicalViewInsetRight, int physicalViewInsetBottom, int physicalViewInsetLeft,
            int systemGestureInsetTop, int systemGestureInsetRight, int systemGestureInsetBottom,
    ...
    private native void nativeDispatchSemanticsAction(long nativePlatformViewId, int id, int action,
            @Nullable ByteBuffer args, int argsPosition);
    ...
    // TODO(mattcarroll): figure out what flags are supported and add javadoc about when/why/where
    // to use this.
    @UiThread
     ...
     */
    @UiThread
    public void runBundleAndSnapshotFromLibrary(@NonNull String bundlePath,
            @Nullable String entrypointFunctionName, @Nullable String pathToEntrypointFunction,
            @NonNull AssetManager assetManager) {
        ensureRunningOnMainThread();
        ensureAttachedToNative();
        nativeRunBundleAndSnapshotFromLibrary(nativePlatformViewId, bundlePath,
                entrypointFunctionName, pathToEntrypointFunction, assetManager);
    }

    private native void nativeRunBundleAndSnapshotFromLibrary(long nativePlatformViewId,
            @NonNull String bundlePath, @Nullable String entrypointFunctionName,
            @Nullable String pathToEntrypointFunction, @NonNull AssetManager manager);
     ...
     * accomplished by passing messages. Messages can be sent from Java to Dart with the
     * corresponding
     ...
     * Flutter context. {@code FlutterJNI} does not know what to do with these messages, so a
     * handler is exposed to allow these messages to be processed in whatever manner is desired:
     *
    ...
    /**
     * Sends an empty reply (identified by {@code responseId}) from Android to Flutter over the
     * given
     * {@code channel}.
     */
    @UiThread
        ...
        if (isAttached()) {
            ...
            Log.w(TAG,
                    "Tried to send a platform message to Flutter, but FlutterJNI was detached from native C++. Could not send. Channel: "
                            + channel + ". Response ID: " + responseId);
        }
    }
    ...
    public void dispatchPlatformMessage(
        ...
        if (isAttached()) {
            nativeDispatchPlatformMessage(
                    nativePlatformViewId, channel, message, position, responseId);
        } else {
            Log.w(TAG,
                    "Tried to send a platform message to Flutter, but FlutterJNI was detached from native C++. Could not send. Channel: "
                            + channel + ". Response ID: " + responseId);
    ...
    private native void nativeDispatchPlatformMessage(long nativePlatformViewId,
            @NonNull String channel, @Nullable ByteBuffer message, int position, int responseId);
    ...
    public interface AccessibilityDelegate {
         ...
         */
        void updateCustomAccessibilityActions(
                @NonNull ByteBuffer buffer, @NonNull String[] strings);

        /**
         * Sends new {@code SemanticsNode} information from Flutter to Android.
         *
         * <p>Implementers are expected to maintain an Android-side cache of Flutter's semantics
         * tree. This method provides updates from Flutter for the Android-side semantics tree
         * cache.
         */
